...
facedetect = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')  # ✅ fixed here
faces_data=[]
...
while True:
    ...
    if not ret:   # ✅ prevent crash if no frame is captured
        print("Failed to grab frame. Try changing camera index (0/1/2).")
        break
...
if 'names.pkl' not in os.listdir('data/'):
    ...
    with open('data/names.pkkl','wb') as f:
        pickle.dump(names,f)
else:
